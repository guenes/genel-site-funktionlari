...
use Genel_Site_Funktionlari\Engine;
...
class Initialize {
	...
	/**
	 * Instance of this Gsf_Is_Methods.
	 *
	 * @var object
	 */
	protected $is = null;
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'genel_site_funktionlari_classes_to_execute', $this->classes );
		...
		foreach ( $this->classes as $class ) {
			try {
				...
				\do_action( 'genel_site_funktionlari_initialize_failed', $err );
	...
	private function get_classes( string $namespace ) {
		...
		$namespace = 'Genel_Site_Funktionlari\\' . $namespace;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'Genel_Site_Funktionlari\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespace ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'Plugin Name is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', GSF_TEXTDOMAIN ) );
			}
